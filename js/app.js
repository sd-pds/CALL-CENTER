...
function saveAuth(){state.adminToken=$('#adminToken').value.trim();localStorage.setItem('proz_admin_token',state.adminToken);showToast('Подключение сохранено');bootstrap().catch(err=>showToast(err.message))}
...
function render(){ const items=applyClientFilters(state.orders); renderStats(items); renderTable(items); renderDetails(items.find(x=>x.id===state.selectedOrderId) || null); }
function renderStats(items){ const newCount=items.filter(x=>x.status==='new').length; $('#statNew').textContent=newCount.toLocaleString('ru-RU'); $('#statTotal').textContent=items.length.toLocaleString('ru-RU'); $('#statRevenue').textContent=formatMoney(items.reduce((s,x)=>s+Number(x.total||0),0)); }
...
function renderTable(items){ const body=$('#ordersBody'); if(!items.length){ body.innerHTML='<tr><td colspan="7" class="muted">Заказов нет</td></tr>'; return; } body.innerHTML=items.map(order=>`<tr data-id="${order.id}" class="${order.id===state.selectedOrderId?'isOn':''}"><td>${statusBadge(order.status)}<div style="margin-top:10px;font-weight:800">#${order.id}</div></td><td><strong>${formatDateTime(order.createdAt)}</strong><div class="muted">${new Date(order.createdAt).toLocaleDateString('ru-RU')}</div></td><td><strong>${order.customer?.name||'—'}</strong><div class="muted">${order.customer?.phone||'—'}</div></td><td><strong>${typeLabel(order)}</strong></td><td><strong>${order.delivery?.address||'—'}</strong><div class="muted">${order.comment||'Без комментария'}</div></td><td><strong>${SITE_LABELS[order.site]||order.site||'—'}</strong><div class="muted">${order.delivery?.restaurant||'—'}</div></td><td class="sumCell">${Math.round(Number(order.total||0))}₽</td></tr>`).join('');
 $$('#ordersBody tr[data-id]').forEach(tr=>tr.addEventListener('click',()=>{state.selectedOrderId=tr.dataset.id; render();})) }
function renderDetails(order){ const wrap=$('#orderDetails'); if(!order){ wrap.className='orderDetails empty'; wrap.textContent='Выбери заказ из таблицы.'; return; } wrap.className='orderDetails'; const items=(order.items||[]).map(it=>`<li>${it.name}${it.weight?` (${it.weight})`:''} ×${it.qty} — ${it.sum} ₽</li>`).join(''); wrap.innerHTML=`<div class="detailCard"><div class="detailTop"><div><h3 style="margin:0">Заказ #${order.id}</h3><div class="muted">${formatDateTime(order.createdAt)} · ${SITE_LABELS[order.site]||order.site||'—'}</div></div><div class="statusRow"><select class="statusSelect" id="statusSelect">${Object.entries(STATUS_LABELS).map(([k,v])=>`<option value="${k}" ${order.status===k?'selected':''}>${v}</option>`).join('')}</select><button class="primaryBtn" id="saveStatusBtn">Сохранить статус</button></div></div><div class="detailGrid"><div class="detailBox"><strong>Клиент</strong><div>${order.customer?.name||'—'}</div><div class="muted">${order.customer?.phone||'—'}</div></div><div class="detailBox"><strong>Получение</strong><div>${order.delivery?.type==='pickup'?'Самовывоз':'Доставка'}</div><div class="muted">${order.delivery?.address||order.delivery?.restaurant||'—'}</div><div class="muted">${order.when?.type==='later' ? 'Предзаказ' : 'Ближайшее время'}</div></div><div class="detailBox"><strong>Оплата</strong><div>${order.paymentLabel||order.payment||'—'}</div><div class="muted">Итого: ${formatMoney(order.total)}</div></div></div><div class="itemsBox"><strong>Состав заказа</strong><ul>${items||'<li>—</li>'}</ul></div></div>`; $('#saveStatusBtn').addEventListener('click', async()=>{const status=$('#statusSelect').value; await api(`/admin/orders/${encodeURIComponent(order.id)}/status`,{method:'POST',body:JSON.stringify({status})}); showToast('Статус обновлён'); await loadOrders(true);}); }
function bindEvents(){ $('#adminToken').value=state.adminToken; $('#saveAuthBtn').addEventListener('click',saveAuth); $('#applyFiltersBtn').addEventListener('click',()=>{syncFilters(); render();}); $('#refreshBtn').addEventListener('click',()=>{syncFilters(); loadOrders().catch(err=>showToast(err.message));}); $('#searchInput').addEventListener('input',()=>{syncFilters(); render();}); $('#siteFilter').addEventListener('change',()=>{syncFilters(); render();}); $('#dateMode').addEventListener('change',()=>{syncFilters(); $('#dateInput').disabled = $('#dateMode').value !== 'date'; render();}); $('#dateInput').addEventListener('change',()=>{syncFilters(); render();}); $$('#statusChips .chip').forEach(btn=>btn.addEventListener('click',()=>{ $$('#statusChips .chip').forEach(x=>x.classList.remove('isOn')); btn.classList.add('isOn'); state.statusFilter=btn.dataset.status||''; render();})); }
...
(function init(){ bindEvents(); if(state.adminToken) bootstrap().catch(err=>showToast(err.message)); })();
